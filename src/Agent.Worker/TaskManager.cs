...
namespace Microsoft.VisualStudio.Services.Agent.Worker
{
    public class TaskLoadResult
    {
        ...
        public IStep PreScopeStep { get; set; }
        public IStep MainScopeStep { get; set; }
        public IStep PostScopeStep { get; set; }
    }
    ...
    {
        // 
        public readonly HashSet<Guid> WellKnownBuiltinTask = new HashSet<Guid>() { };
        public async Task DownloadAsync(IExecutionContext executionContext, IEnumerable<Pipelines.JobStep> steps)
        {
            ...
            IEnumerable<Pipelines.TaskStep> uniqueTasks =
                from task in tasks
                where task.Enabled && !WellKnownBuiltinTask.Contains(task.Reference.Id)  // Do we still need filter disable task?
                group task by new
                {
                    task.Reference.Id,
        ...
        public TaskLoadResult Load(IExecutionContext executionContext, Pipelines.TaskStep task)
        {
            ...
            if (definition.Data?.PreJobExecution != null)
            {
                ...
                taskRunner.Stage = JobRunStage.PreScope;
            ...
            if (definition.Data?.Execution != null)
            {
                ...
                var taskRunner = HostContext.CreateService<ITaskRunner>();
                taskRunner.Stage = JobRunStage.MainScope;
                taskRunner.Task = task;
            ...
            if (definition.Data?.PostJobExecution != null)
            {
                ...
                var taskRunner = HostContext.CreateService<ITaskRunner>();
                taskRunner.Stage = JobRunStage.PostScope;
                taskRunner.Task = task;
