    ...
    public sealed class Worker : AgentService, IWorker
    {
        ...
        public async Task<int> RunAsync(string pipeIn, string pipeOut)
        {
            ...
            using (var channelTokenSource = new CancellationTokenSource())
            {
                ...
                var jobMessage = JsonUtility.FromString<Microsoft.TeamFoundation.DistributedTask.Pipelines.AgentJobRequestMessage>(channelMessage.Body);
        ...
        private void InitializeSecretMasker(Microsoft.TeamFoundation.DistributedTask.Pipelines.AgentJobRequestMessage message)
        {
            Trace.Entering();
            ...
            foreach (ServiceEndpoint endpoint in message.Endpoints ?? new List<ServiceEndpoint>())
            {
        ...
        private void SetCulture(Microsoft.TeamFoundation.DistributedTask.Pipelines.AgentJobRequestMessage message)
        {
