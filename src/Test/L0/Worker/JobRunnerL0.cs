    ...
    public sealed class JobRunnerL0
    {
        ...
        public async Task JobExtensionInitializeFailure()
        {
            using (TestHostContext hc = CreateTestContext())
            {
                ...
                _stepRunner.Verify(x => x.RunAsync(It.IsAny<IExecutionContext>(), It.IsAny<IList<IStep>>(), JobRunStage.PreScope), Times.Never);
            }
        }
        ...
        public async Task SkipJobStepsWhenPreJobStepsFail()
        {
            using (TestHostContext hc = CreateTestContext())
            {
                _stepRunner.Setup(x => x.RunAsync(_jobEc, _initResult.PreJobSteps, JobRunStage.PreScope))
                ...
                Assert.Equal(TaskResult.Failed, _jobEc.Result);
                _stepRunner.Verify(x => x.RunAsync(It.IsAny<IExecutionContext>(), It.Is<IList<IStep>>(s => s.Equals(_initResult.PreJobSteps)), JobRunStage.PreScope), Times.Once);
        ...
        public async Task RunPostJobStepsEvenPreJobStepsFail()
        {
            using (TestHostContext hc = CreateTestContext())
            {
                ...
                Assert.Equal(TaskResult.Failed, _jobEc.Result);
                _stepRunner.Verify(x => x.RunAsync(It.IsAny<IExecutionContext>(), It.Is<IList<IStep>>(s => s.Equals(_initResult.PreJobSteps)), JobRunStage.PreScope), Times.Once);
                _stepRunner.Verify(x => x.RunAsync(It.IsAny<IExecutionContext>(), It.Is<IList<IStep>>(s => s.Equals(_initResult.PostJobStep)), JobRunStage.PostScope), Times.Once);
            }
        }
